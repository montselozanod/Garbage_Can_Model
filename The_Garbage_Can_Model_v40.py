...
# Knowledge distribution among members
K0 = np.ones(m) # equal (everyone is experienced)
K1 = np.arange(1, 0, -.1)*sol_coeff
K2 = np.ones(m)*.1 # few
Knowledge = np.zeros((3,v))
Knowledge[0] = K0 # experienced
Knowledge[1] = K1 # mixed
Knowledge[2] = K2 # inexperienced
...
def garbage_can(A, D, E, nel, K, Entry_m, Entry_w):
    '''
        ...
        for c1 in np.arange(v):  # cycle over decision makers
            ...
            if len(Values) > 0:
                ...
                Choices_energy_spent_calc[best_choice_index] =\
                    Choices_energy_spent_calc[best_choice_index] + E[c1] - K[c1]
            else:
    ...
    for x2 in np.arange(3):  # Energy dist: increasing, equall, decreasing
        for x3 in np.arange(3):  # problem access A: all, triangular, diagonal
            for x4 in np.arange(3):  # D: choices to dec. mkrs: all, tri, diag
                for x5 in np.arange(3): # K: knowledge distribution
                    ...
                    for i101 in np.arange(iterations):
                        ...
                        Choices, Problems, Members, Choices_energy_required,\
                            Choices_energy_spent =\
                            garbage_can(A, D, E, nel, K, Entry_m1, Entry_w1)
